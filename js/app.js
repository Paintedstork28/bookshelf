...
function getFilteredBooks() {
  ...
  let filtered = allBooks.filter(book => {
    if (search && !book.title.toLowerCase().includes(search) && !book.author.toLowerCase().includes(search) && !(book.tags || []).some(t => t.toLowerCase().includes(search)) && !(book.reviewSpoilerFree || '').toLowerCase().includes(search) && !(book.reviewSpoiler || '').toLowerCase().includes(search) && !book.category.toLowerCase().includes(search)) return false;
